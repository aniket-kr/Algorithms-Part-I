...
/**
 * Sorts the array using mergesort. The array is sorted using an auxiliary array of size
 * <code>n</code>. Uses time proportional to <code>&theta;(nlog<sub>2</sub>n)</code> and
 * no additional space proportional to <code>n</code>, where <code>n</code> is the number
 * of elements to sort. The sort is stable.
 *
 * @author Aniket Kumar
 */
