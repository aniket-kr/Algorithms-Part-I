...
/**
 * A minimum oriented priority queue is an ordered collection of objects that
 * supports efficient operations to <em>insert</em>, <em>delete</em> and
 * <em>find-the-minimum</em> key(s). This implementation also supports the above
 * operations.
 *
 * <p>Apart from these, the {@link MaxPQ} also supports iteration over keys in
 * ascending order, using time and space proportional to <code>&theta;(n)</code>.</p>
 *
 * @param <K> The type of keys in the min-oriented PQ that implement either a natural
 *            order using {@link Comparable} interface or have an accompanying
 *            {@link java.util.Comparator} passed to the constructor.
 * @author Aniket Kumar
 */
public class MinPQ<K> implements Collection<K> {
  ...
  /**
   * Delete and return the minimum key from the priority queue.
   * 
  ...
  /**
   * Return the minimum key from the priority queue, without removing it.
   * 
   * @return The minimum most key in the priority queue.
   * @throws java.util.NoSuchElementException If the priority queue is empty (underflow).
   */
  public K min() {
    return null;
